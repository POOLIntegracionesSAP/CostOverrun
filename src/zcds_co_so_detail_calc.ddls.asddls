...
@EndUserText.label: 'Cost Overrun for SO - Detail Calc'
@Metadata.allowExtensions: true

// CDS view to select the corresponding values from the CDS of the Customizing table and to calculate the Cost Overrun
define view entity ZCDS_CO_SO_DETAIL_CALC as select from ZCDS_CO_CUSTO as Custo
...
{
// The primary keys are Sales Order and SO Item related to the Journal Entry, and the Company Code, Type and Classification defined for the GLAccount in the Customizing table
    @EndUserText.label: 'Type'
key Custo.Type as type,
    @EndUserText.label: 'Classification'
key Custo.Classification as classification,
    @EndUserText.label: 'Sales Order'
key _JournalEntries.salesdoc as salesdoc,
    @EndUserText.label: 'Sales Order Item'
key _JournalEntries.salesdocitem as salesdocitem,
...
// In such cases, all posting amounts for the same SO Item  will be summed
...
// All actual amounts referenced to the same SO Item will be summed, grouped by Type and Classification
...
// All plan amounts referenced to the same SO Item  will be summed, grouped by Type and Classification
...
group by _JournalEntries.salesdoc, _JournalEntries.salesdocitem, Custo.CompanyCode, Custo.Glaccount, _JournalEntries.companycodecurrency, Custo.Actuals,
         Custo.Plans, Custo.Type, Custo.Classification
// Condition to remove the Journal Entries that are not associated to any sales order
having
    count(distinct _JournalEntries.salesdoc) != 0
